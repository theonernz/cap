...
const EntityManager = {
    ...
    createAISeagull(x, y, size, power) {
        ...
        return {
            ...
            name: `${TRANSLATIONS[CONFIG.language].aiSeagullPrefix}${Math.floor(Math.random() * 1000)}`,
    ...
    createSpoiledScallop(x, y) {
        ...
        return {
            ...
            isKing: false,
            birthTime: Date.now(),
            spawnTime: Date.now(),  // 用于追踪变质扇贝的生命周期
    ...
    getLocalPlayer() {
        // 在多人模式中，查找可控的玩家
        const controllable = this.players.find(p => p.isControllable);
